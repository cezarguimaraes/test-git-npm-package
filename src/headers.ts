...
import { CACHING_HEADERS, COMMON_BUNDLES, IMMUTABLE_CACHING_HEADER, PAGE_DATA_DIR } from './constants'

function preloadHeadersByPath(
  pages: Page[],
  manifest: Manifest,
  pathPrefix: string): PathHeadersMap {
...
function cacheHeadersByPath(pages: Page[], manifest: Manifest): PathHeadersMap {
  ...
  const files = ([] as string[]).concat(...chunks.map(chunk => manifest[chunk] || []))

  return Object.fromEntries(
    files.map(file => ['/' + file, [IMMUTABLE_CACHING_HEADER]])
...
function linkTemplate(asset: string, pathPrefix: string, type: 'script' | 'fetch' = 'script') {
...
function applyUserHeadersTransform(
  ...
  transform: (headers: string[], path: string) => string[]): PathHeadersMap {
